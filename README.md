...
- **Средняя наработка до отказа (Mean Time To Failure)** – математическое ожидание наработки объекта до первого отказа
- **Средняя наработка на отказ (Mean Time Between Failures)** – отношение суммарной наработки к математическому ожиданию числа отказов в течение этой наработки
...
### Динамические методы
Используют результаты выполнения программы:
1. Обнаруженные отказы
2. Время работы программы при каждом запуске
3. Результаты тестов и трассы выполнения программы

### Статические методы
Основаны на анализе различных артефактов процесса проектирования:
1. Спецификации
2. Архитектура
3. Исходные коды программы

### Архитектурные методы
Основаны на анализе архитектуры системы и могут использовать как динамические, так и статические подходы.

**Архитектурные методы выполняют:**
1. Декомпозицию программной системы на компоненты
2. Оценку надежности каждого компонента в отдельности
3. Определение взаимных влияний компонентов
4. Формирование общей оценки надежности всей системы
...
### Эмпирические методы
Используют информацию о процессе проектирования:
1. Организация процесса проектирования, сертификаты, уровень зрелости
...
## Раздувальщики
Раздувальщики представляют код, методы и классы, которые раздулись до таких больших размеров, что с ними стало невозможно эффективно работать. Все эти запахи зачастую не появляются сразу, а нарастают в процессе эволюции программы.
...
### Primitive Obsession (Одержимость элементарными типами)
...
### Refused Bequest (Отказ от наследства)
...
### Divergent Change (Расходящиеся модификации)
...
### Speculative Generality (Теоретическая общность)
...
### Feature Envy (Завистливые функции)
...
### Incomplete Library Class (Неполнота библиотечного класса)
